...
def _record_start(runtime, stage, name, *, span_id, input_summary, node_id=None):
    """Emits the "stage_start" half of the stage_start/stage_complete pair
    _record (below) completes -- correlated by span_id, generated by the
    caller from the OTel span it already has open, so both halves share the
    same id. No DB write here; the trajectory row is written once, at
    completion, by _record. `node_id` disambiguates which plan node this
    belongs to when multiple subagents are in flight in the same wave (None
    for whole-run stages like plan/synthesis/reflection)."""
...
def _record(runtime, stage, name, *, input_summary, output, usage, latency_ms, start_dt, end_dt, span_id, node_id=None):
    ctx = runtime.context
...
# Forces the "search" tool on the ReAct loop's first step (see agent_node
# below). Must be the explicit OpenAI dict form, not the bare tool-name
# string LangChain also documents as equivalent shorthand -- confirmed live
# against this project's actual provider/model (google/gemini-2.5-flash via
# OpenRouter): the bare string "search" was silently ignored (model still
# returned zero tool_calls), while this dict form reliably forces the call.
_FORCE_SEARCH_TOOL_CHOICE = {"type": "function", "function": {"name": "search"}}
...
async def agent_node(state: dict, runtime: Runtime[AgentContext]) -> dict:
    ...
    with stage_span(f"agent_step:{state.get('step', 0)}"):
        ...
        _record_start(
            ...
            span_id=span_id, input_summary={"n_messages": len(state["messages"])},
            node_id=ctx.source_id_prefix or None,
        )
        # Force an actual search on the very first turn. Confirmed live: the
        # model sometimes narrates the right plan in `content` ("I need to
        # find X, then Y...") but attaches zero tool_calls to that same
        # response -- _agent_route (below) then reads "no tool_calls" as
        # "ready to finalize" and the loop ends after one step with nothing
        # retrieved, so finalize_finding is called against empty evidence.
        # That's never a legitimate step-0 outcome: finalize_finding_v1.txt
        # treats "no passages retrieved" as a failure mode, and
        # react_agent_v1.txt already forbids answering from prior knowledge
        # -- so there is no question this loop could correctly answer
        # without searching at least once. Steps after 0 are left on "auto"
        # tool_choice: by then the model has real evidence to judge whether
        # another hop is needed, which is a legitimate call for it to make.
        model = ctx.chat_model.bind(tool_choice=_FORCE_SEARCH_TOOL_CHOICE) if state.get("step", 0) == 0 else ctx.chat_model
